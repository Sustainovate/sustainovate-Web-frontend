...
export function EventCard({
  ...
  const chips = [...(tags || []), ...(category ? [category] : [])];

  const start = new Date(date);
  let durationLabel = `${durationDays} day${durationDays > 1 ? "s" : ""}`;
  if (durationDays < 1) {
    // If less than 1 day, show hours instead
  ...
  return (
    <div className="w-full rounded-xl border border-neutral-800 bg-neutral-900 text-gray-100 overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-200">
            ...
            {chips.length > 0 && (
              <div className="flex flex-wrap items-center gap-2">
                {chips.map((c) => (
                  <span
                    key={c}
                    className="rounded-full bg-neutral-800 px-3 py-1 text-[10px] sm:text-xs text-gray-300 border border-neutral-700"
                  >
                    {c}
                  </span>
                ))}
